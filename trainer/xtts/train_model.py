...
def train_xtts(config):
    ...
    try:
        ...
        gpt_config = config.gpt if hasattr(config, 'gpt') else config
        ...
        logger.info("  • Repetition penalty: %.2f", gpt_config.repetition_penalty)
        logger.info("  • Temperature: %.2f", gpt_config.temperature)
