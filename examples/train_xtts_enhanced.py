...
def create_enhanced_config(args):
    ...
    # Create enhanced GPT trainer config
    gpt_config = EnhancedGPTTrainerConfig()
    
    # Apply audio length settings
    gpt_config.max_wav_length = int(args.max_audio_length * 22050)  # Convert seconds to samples
    
    # Apply language control settings
    gpt_config.temperature = args.temperature
    gpt_config.repetition_penalty = args.repetition_penalty
...
def print_enhancement_summary(config):
    ...
    print("\n📢 AUDIO CUTOFF FIXES:")
    print(f"  • Max audio length: {config.gpt.max_wav_length / 22050:.1f}s")
    print(f"  • Max audio tokens: {config.gpt.gpt_max_audio_tokens}")
    print(f"  • Max text length: {config.gpt.max_text_length}")
    
    print("\n🌍 LANGUAGE MIXING FIXES:")
    print(f"  • Repetition penalty: {config.gpt.repetition_penalty}")
    print(f"  • Temperature: {config.gpt.temperature}")
    print(f"  • Top-k sampling: {config.gpt.top_k}")
    print(f"  • Top-p sampling: {config.gpt.top_p}")
